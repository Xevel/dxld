 ...
 *      RingBufferByte_t Buffer;
 *      uint8_t      BufferData[128];
 *
 *      // Initialize the buffer with the created storage array
 *      RingBufferByte_InitBuffer(&Buffer, BufferData, sizeof(BufferData));
 *
 *      // Insert some data into the buffer
 *      RingBufferByte_Insert(Buffer, 'H');
 *      RingBufferByte_Insert(Buffer, 'E');
 *      RingBufferByte_Insert(Buffer, 'L');
 *      RingBufferByte_Insert(Buffer, 'L');
 *      RingBufferByte_Insert(Buffer, 'O');
 *
 *      // Cache the number of stored bytes in the buffer
 *      uint32_t BufferCount = RingBufferByte_GetCount(&Buffer);
 *
 ...
 *      while (BufferCount--)
 *        putc(RingBufferByte_Remove(&Buffer));
 *  \endcode
 *
 *  @{
 */
...
#define __RING_BUFFER_H__
	...
	/* Type Defines: */
		 ...
		 *  \ref RingBufferByte_InitBuffer() before use.
		 */
		typedef struct
		{
	...
	/* Inline Functions: */
		 ...
		 */
		static inline void RingBufferByte_InitBuffer(RingBufferByte_t* Buffer,
...
//			SetGlobalInterruptMask(CurrentGlobalInt);
		 ...
		 */
		static inline uint32_t RingBufferByte_GetCount(RingBufferByte_t* const Buffer)
		{
			uint32_t Count;
...
//			SetGlobalInterruptMask(CurrentGlobalInt);
		 ...
		 */
		static inline uint32_t RingBufferByte_GetFreeCount(RingBufferByte_t* const Buffer)
		{
			return (Buffer->Size - RingBufferByte_GetCount(Buffer));
		}
		 ...
		 *  buffer (via a call to the \ref RingBufferByte_GetCount() function) in a temporary variable
		 ...
		 */
		static inline bool RingBufferByte_IsEmpty(RingBufferByte_t* const Buffer)
		{
			return (RingBufferByte_GetCount(Buffer) == 0);
		}
		 ...
		 *  \return Boolean \c true if the buffer contains no free space, \c false otherwise.
		 */
		static inline bool RingBufferByte_IsFull(RingBufferByte_t* const Buffer)
		{
			return (RingBufferByte_GetCount(Buffer) == Buffer->Size);
		}
		 ...
		 */
		static inline void RingBufferByte_Insert(RingBufferByte_t* Buffer, const uint8_t Data)
		{
			//GCC_FORCE_POINTER_ACCESS(Buffer);
...
//			SetGlobalInterruptMask(CurrentGlobalInt);
		 ...
		 */
		static inline uint8_t RingBufferByte_Remove(RingBufferByte_t* Buffer)
		{
//			GCC_FORCE_POINTER_ACCESS(Buffer);
...
//			SetGlobalInterruptMask(CurrentGlobalInt);
		 ...
		 *  \return Next data element stored in the buffer.
		 */
		static inline uint8_t RingBufferByte_Peek(RingBufferByte_t* const Buffer)
		{
			return *Buffer->Out;
		}
